...
function C08({ viewport, skin, earnings, payments }) {
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ fontSize: 9, fontWeight: 500, color: '#6ee7b7', letterSpacing: '0.08em', textTransform: 'uppercase' }}>#08</span>
            <span style={{ fontSize: 14, fontWeight: 500, color: c.br || '#e0e8f0', letterSpacing: '-0.03em' }}>Payment ledger</span>
          </div>
        ...
        <div style={{ fontSize: 9, color: c.dim, marginTop: 8 }}>3-tier payment flow: pending → confirmed → paid</div>
  
      </div>
    </div>
  );
}
export default memo(C08);