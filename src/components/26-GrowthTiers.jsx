...
function C26({ viewport, skin, workers }) {
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ fontSize: 9, fontWeight: 500, color: '#fbbf24', letterSpacing: '0.08em', textTransform: 'uppercase' }}>#26</span>
            <span style={{ fontSize: 14, fontWeight: 500, color: c.br || '#e0e8f0', letterSpacing: '-0.03em' }}>Growth tiers</span>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: mob ? 'repeat(2,1fr)' : 'repeat(4,1fr)', gap: 6 }}>
          {Object.entries(workers || {}).slice(0, 8).map(([k, v]) => (
        ...
        <div style={{ fontSize: 9, color: c.dim, marginTop: 8 }}>Bootstrap → Growth → Enterprise → Titan roadmap</div>
  
      </div>
    </div>
  );
}
export default memo(C26);