...
function C18({ viewport, skin, earnings, network, fleet }) {
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ fontSize: 9, fontWeight: 500, color: '#fbbf24', letterSpacing: '0.08em', textTransform: 'uppercase' }}>#18</span>
            <span style={{ fontSize: 14, fontWeight: 500, color: c.br || '#e0e8f0', letterSpacing: '-0.03em' }}>Profitability model</span>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: mob ? 'repeat(2,1fr)' : 'repeat(4,1fr)', gap: 6 }}>
          {Object.entries(earnings || {}).slice(0, 8).map(([k, v]) => (
        ...
        <div style={{ fontSize: 9, color: c.dim, marginTop: 8 }}>Break-even analysis with fee sensitivity</div>
  
      </div>
    </div>
  );
}
export default memo(C18);